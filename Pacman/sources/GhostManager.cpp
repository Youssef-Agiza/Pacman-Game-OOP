...
void GhostManager::createGhost(Board* board,Graph* graph,Pacman* pacman, int speedTimer) //create four ghosts and adds them to ghost manager
{
	...
	Blinky->setIntialRow(15).setIntialCol(14);
	...
	//Pinky
	Ghost* Pinky = new Ghost(15, 13, board->getTileSize(), board);
	Pinky->resetPosition();
	...
	//Inky
	Ghost* Inky = new Ghost(15, 12, board->getTileSize(), board);
	Inky->resetPosition();
	...
	//Clyde
	Ghost* Clyde = new Ghost(15, 14, board->getTileSize(), board);
	Clyde->resetPosition();
